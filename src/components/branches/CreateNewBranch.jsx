...
export default function CreateNewBranch() {
...

    {
      ...
      renderCell: ({ row, id }) => (
        <div className=" flex space-x-2 justify-center">
          <div
            onClick={() => {
              setShowModal({ status: true, id: rows[id], type: "edit" });
            }}
            className={`
  ...
  async function getBranches() {
    setLoading(true);
    const response = await axios.get("/api/branches/get-branches");
  ...
  async function handleDeleteBranch(id) {
    ...
    setLoading(true);
    try {
      const response = await axios.post("/api/branches/delete-branch", {
        code: id,
      });
      setLoading(false);
      ...
      <Modal
        title={
          <div className="uppercase font-semibold">
            {showModal.type === "edit" ? "Edit branch" : "Add New Branch"}
          </div>
        }
        opened={showModal.status}
        onClose={() => {
          setShowModal({ status: false, type: "", id: "" });
        }}
        closeOnClickOutside={false}
        size={screenWidth < 768 ? "100%" : "40%"}
      >
        <NewBranch
          id={showModal.id}
          type={showModal.type}
      ...
      >
        <div className={`${poppins?.className} text-sm `}>
          ...
          <div className="p-5  py-3 rounded ">
            Are you sure you want to delete branch?.. <br />
            ...
            <div className="flex justify-end space-x-2 mt-2">
              ...
              <button
                ...
                onClick={() => {
                  handleDeleteBranch(showDialog.content);
                }}
              >
                Yes
              </button>
